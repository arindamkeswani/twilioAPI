...
module.exports.sendOTP = function sendOTP(req, res) {
      ...
      client.calls.create({
            twiml: `<Response><Say>Your OTP is ${OTPtoBeSent[0],OTPtoBeSent[1],OTPtoBeSent[2],OTPtoBeSent[3],OTPtoBeSent[4],OTPtoBeSent[5]}</Say></Response>`,
            to: phone,
            from: process.env.SENDER_NUMBER //process.env.SENDER_NUMBER
      }, function (err, call) {
            if (err) {
                  console.log(err);
            } else {
